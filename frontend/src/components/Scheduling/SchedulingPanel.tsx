...
const SchedulingPanel: React.FC<SchedulingPanelProps> = ({ projectId }) => {
  ...
  // API-Aufruf zum Abrufen des Zeitplans
  const { data: scheduleData, isLoading, error } = useQuery({
    queryKey: ['schedule', projectId],
    queryFn: async () => {
      const response = await apiClient.post(`/api/v1/scheduling/projects/${projectId}/schedule`);
  ...
  // Validierung des Zeitplans
  const validateSchedule = async (scheduleData: Schedule['schedule']) => {
    try {
      const response = await apiClient.post(
        `/api/v1/scheduling/projects/${projectId}/validate-schedule`,
        { schedule: scheduleData }
      );
      setValidation(response.data);
    } catch (error) {
      console.error('Fehler bei der Validierung des Zeitplans:', error);
    }
  };
  ...
  if (error) {
    return (
      <div className="p-4 bg-red-50 border border-red-200 rounded-lg" role="alert">
        <p className="text-red-600">Fehler beim Laden des Zeitplans. Bitte versuchen Sie es später erneut.</p>
      </div>
    );
  }

  // Kein Zeitplan verfügbar
  if (!schedule) {
    return (
      <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
        <p className="text-gray-600">Kein Zeitplan verfügbar.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="px-4 py-5 sm:p-6">
          <dl className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              ...
              <dd className="mt-1 text-sm text-gray-900">{schedule.total_duration_days} Tage</dd>
            </div>
          </dl>
        </div>
      </div>
      ...
      {schedule.schedule.length > 0 ? (
        schedule.schedule.map((slot, index) => (
          ...
          >
            <div className="flex justify-between items-start">
              <div>
                <p className="text-sm font-medium text-gray-900">{slot.description}</p>
        ...
        <p className="text-gray-600">Keine Zeitplandaten verfügbar.</p>
      )}
      ...
      {validation && (validation.conflicts.length > 0 || validation.warnings.length > 0) && (
        <div className="bg-white shadow overflow-hidden sm:rounded-lg">
          <div className="px-4 py-5 sm:p-6">
            <h4 className="text-base font-medium text-gray-900 mb-4">Validierungsergebnisse</h4>
